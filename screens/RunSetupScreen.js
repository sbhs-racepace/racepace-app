...
export default class RunSetupScreen extends React.Component {
  ...
  async getRouteFromAddress(start, end) {
    ...
    let { latitude: s_lat, longitude: s_lon } = (await Location.geocodeAsync(
      start + ',' + global.region.name
    ))[0];
    let startCoord = `${s_lat},${s_lon}`;
    let { latitude: e_lat, longitude: e_lon } = (await Location.geocodeAsync(
      end + ',' + global.region.name
    ))[0];
    ...
    return (
      <View style={{ flex: 1, backgroundColor: Color.lightBackground }}>
        ...
        <View style={[STYLES.container, { flex: 1, alignItems: 'center' }]}>
          <Text style={[STYLES.text_style, { textAlign: 'center' }]}>
            Route Information
          </Text>
          <Text style={STYLES.text_style}>
            Time: {this.state.time.minutes} minutes {this.state.time.seconds}{' '}
            seconds
          </Text>
          <Text style={STYLES.text_style}>
          ...
          <Text style={STYLES.text_style}>
            Calories Burnt/Kilojoules Burnt: {this.state.calories} Cal/{' '}
            {Math.floor(this.state.calories * 4.184)} Kj
          </Text>
